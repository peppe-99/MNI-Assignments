...
__global__ void prodottoScalare2(float *a, float *b, float *c, int n);
...
int main(void) {
    ...
    prodottoScalare2<<<gridDim, blockDim, blockDim.x>>>(a_dev, b_dev, c_dev, n);
...
__global__ void prodottoScalare2(float *a, float *b, float *c, int n) {
    /* vettore shared conterrà i prodotti effettuati dai thread di un blocco */
